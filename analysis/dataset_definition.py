...
def make_dataset_opioids(index_date, end_date):
    ...
    dataset.par_opioid_any = has_med_event(
        codelists.par_opioid_codes
    )  # Parenteral opioid
    dataset.trans_opioid_any = has_med_event(
        codelists.trans_opioid_codes
    )  # Transdermal opioid
    dataset.oth_opioid_any = has_med_event(
        codelists.oth_opioid_codes
    )  # Other admin route opioid

    # By strength/type
    dataset.hi_opioid_any = has_med_event(
        codelists.hi_opioid_codes
...
def registrations(start_date, end_date):
    return practice_registrations.where(
        ...
        & (
            practice_registrations.end_date.is_after(end_date)
